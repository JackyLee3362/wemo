...
from database.sns import Sns
from wemo.database.sns import Comment
from wemo.database.micro_msg import Contact
from ..update.avatar_updater import AvatarUpdater
from ..update.emoji_updater import EmojiUpdater
from ..update.img_updater import ImageUpdater
from ..update.video_updater import VideoUpdater
...
class HtmlExporter(threading.Thread):
    ...
    def __init__(
        ...
        self.avatar_exporter = AvatarUpdater(dir_name)
        self.image_exporter = ImageUpdater(dir_name)
        self.video_exporter = VideoUpdater(dir_name)
        self.html_head = None
        self.html_end = None
        self.file = None
    ...
    def run(self) -> None:
        ...
        if self.gui.account_info and self.gui.account_info.get("wxid"):
            self.avatar_exporter.update_by_username(self.wxid)
        ...
        if cover_url:
            cover_path = self.image_exporter.save_server_img(
                (cover_url, "", ""), "image"
            )
        ...
        message_datas = self.db_sns.get_feeds_by_dur_and_wxids(begin_time, end_time)
    ...
    def export_msg(
        ...
        # 头像路径
        avatar_path = self.avatar_exporter.update_by_username(username)

        contact = contacts_map.get(username)
        # 备注， 或用户名
        remark = contact.remark if contact.remark else contact.nick_name

        # 朋友圈图片
        images = self.image_exporter.update_by_moment(msg)

        # 朋友圈视频
        videos = self.video_exporter.update_by_moment(msg)
        ...
        if msg.timelineObject.contentDesc:
            ...
            content_desc = EmojiUpdater.replace_emoji(content_desc)
