...
from config.user_init import WX_DIR
from decrypter import db_decrypt
from config import APP_USER_CACHE_DIR, APP_USER_DB_DIR, LOG
...
class DataBaseSet:
    ...
    def init_db_set(self):
        # 1. 初始化用户数据库
        if not APP_USER_DB_DIR.exists():
            os.mkdir(APP_USER_DB_DIR)
        # 2. 清除缓存数据库
        self.init_cache()
        # 3. 将解密后的数据放入缓存数据库
    ...
    def update_user_db_by_cache(self, db: str):
        user_db = APP_USER_DB_DIR.joinpath(db + ".db")
        cache_db = APP_USER_CACHE_DIR.joinpath(db + ".db")
        # 情况一：缓存数据库不存在
    ...
    def init_cache(self):
        ...
        if not APP_USER_CACHE_DIR.exists():
            os.mkdir(APP_USER_CACHE_DIR)
        if WX_DIR is None:
            return
        if WX_DIR.exists() and APP_USER_CACHE_DIR.exists():
            # todo: 如果有数据 warning
            shutil.rmtree(APP_USER_CACHE_DIR)
            os.mkdir(APP_USER_CACHE_DIR)
