...
class ImageDecrypter(Decrypter):
    ...
    def decrypt_images(self, start: date, end: date) -> None:
        ...
        """
        year_month_list = self.get_months_between_dates(start, end)

        # 初始化创建路径
        for year_month in year_month_list:
            src_dir = self.src_dir.joinpath(year_month)
            dst_img_dir = self.src_dir.joinpath(year_month)
            dst_thm_dir = self.src_dir.joinpath(year_month)
            dst_img_dir.joinpath(year_month).mkdir(exist_ok=True)
            dst_thm_dir.joinpath(year_month).mkdir(exist_ok=True)
            for file in src_dir.rglob("*"):
                if file.is_file() and not file.name.endswith("_t"):
                    self.handle_file(year_month, file)

    def handle_file(self, year_month: str, file: Path) -> None:
        """
        ...
        thm_dst = self.src_dir.joinpath(year_month, thm_name)
        img_dst = self.src_dir.joinpath(year_month, img_name)
        # 如果目标目录已存在，则跳过
        if img_dst.exists():
